...
func ListenUDP(c conn, cfg Config) (*UDP, error) {
	udp, err := newUDP(c, cfg)
	if err != nil {
		return nil, err
	}
	log.Info("UDP listener up", "self", udp.self)
	// realaddr := c.LocalAddr().(*net.UDPAddr)
	//	log.Info("Config nat map", "realaddr.Port", realaddr.Port, "realaddr.Port", realaddr.Port)
	//	go nat.Map(nat.Any(), nil, "udp", realaddr.Port, realaddr.Port, "ethereum discovery")
	//	time.Sleep(5 * time.Second)

	return udp, nil
}

func newUDP(c conn, cfg Config) (*UDP, error) {
	ourAddr := c.LocalAddr().(*net.UDPAddr)
	...
	udp := &UDP{
		...
		self:        NewNode(PubkeyID(&cfg.PrivateKey.PublicKey), ourAddr.IP, uint16(ourAddr.Port), uint16(ourAddr.Port)),
	}
	realaddr := c.LocalAddr().(*net.UDPAddr)
...
func (t *UDP) Findnode(toid NodeID, toaddr *net.UDPAddr, target NodeID) ([]*Node, error) {
	...
	errc := t.pending(toid, neighborsPacket, func(r interface{}) bool {
		reply := r.(*neighbors)
		log.Trace("recieve neighbors packet", "reply", reply)
...
func (t *UDP) loop() {
	...
	for {
		...
		case r := <-t.gotreply:
			...
			for el := plist.Front(); el != nil; el = el.Next() {
				p := el.Value.(*pending)
				log.Debug("check match", "p.from", p.from, "r.from", r.from.String(), "p.ptype", p.ptype, "r.ptype", r.ptype)
...
func (req *ping) handle(t *UDP, from *net.UDPAddr, fromID NodeID, mac []byte) error {
	...
	if !t.handleReply(fromID, pingPacket, req) {
		...
		log.Debug("handle reply", "fromID", fromID, "from", from)
	}
	return nil
}
